...
import (
	...
	toolkit2 "shortlink/internal/common/toolkit"
	"time"
)
...
func NewLink(
	...
	favicon := toolkit2.GetFaviconWithDefault(originalUrl, config.DefaultFavicon.String())
...
func verifyWhiteList(originUrl string) error {
	...
	domain := toolkit2.ExtractDomain(originUrl)
	if domain == "" {
...
func (l Link) Enable() {
	l.enableStatus = constant.StatusEnable
}
func (l Link) Disable() {
	l.enableStatus = constant.StatusDisable
}
...
func (l Link) GenUniqueShortUri(attempts int, ifExistsFunc func(string) bool) error {
	for i := 0; i < attempts; i++ {
		shortUri := toolkit2.HashToBase62(l.originalUrl)
...
func (l Link) genShortUrl() {
	...
	l.fullShortUrl = "https://" + l.domain + "/" + toolkit2.HashToBase62(originalUrl)
}
