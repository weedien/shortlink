...
type Link struct {
	...
	ValidType   int            `gorm:"column:valid_type;not null;comment:有效期类型 0：永久有效 1：自定义" json:"valid_date_type"`
	StartDate   time.Time      `gorm:"column:start_date;not null;comment:有效期开始时间" json:"valid_start_date"`
	EndDate     time.Time      `gorm:"column:end_date;not null;comment:有效期结束时间" json:"valid_end_date"`
